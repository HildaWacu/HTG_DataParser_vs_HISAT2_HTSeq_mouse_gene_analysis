...
# # ## install IGV from Bioconda in the mscproject
conda install -c bioconda igv

## check if IGV has been successfully installed
if  ! conda list | grep "igv" -y
    then
        echo Error: IGV package does not exits.
        # echo $?
        exit 1 ## once it encounters a situation where this condition is met, the Bash script exits and commands cease to be executed
    else 
        echo IGV successfully installed
fi
