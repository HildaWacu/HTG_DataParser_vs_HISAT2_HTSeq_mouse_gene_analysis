...
```{r}
hist(counts(ddsHTG), ylim = c(0,5000))
hist(counts(dds_cpm_filtered_HTG1), ylim = c(0,5000))
...
qqnorm(counts(dds_cpm_filtered_HTG1), ylim = c(0,5000))
qqline(counts(dds_cpm_filtered_HTG1), ylim = c(0,5000))
# hist(rowMeans(log2(cpm(counts(dds)) + 1)))
```
...
designMatrix <- model.matrix(~ 0 + condition2, data = coldata)
head(designMatrix)
```
...
```{r}
contrastMatrixCol <- makeContrasts(condition2colitisOnly - condition2noColitis, levels = designMatrix)
...
countTable <- as.data.frame(counts(ddsHTG))
...
nrow(sigRes_HTG); sum(nrow(subset(sigRes_HTG,sigRes_HTG$logFC > 1)),
nrow(subset(sigRes_HTG,sigRes_HTG$logFC < 1)))


for (i in sigRes_HTG$logFC) {
  # print(i)
  if (i > 1) {
...
sigRes_HTG_reg <- cbind(sigRes_HTG, deg_reg)
nrow(sigRes_HTG)
```
...
```{r}
contrastMatrixColnodysp <- makeContrasts(condition2colitis_nodysplasia - condition2noColitis, levels = designMatrix)
...
#to confirm the number of DEG is correct
nrow(sigRes_HTG_Colnodysp); sum(nrow(subset(sigRes_HTG_Colnodysp,sigRes_HTG_Colnodysp$logFC > 1)),
nrow(subset(sigRes_HTG_Colnodysp,sigRes_HTG_Colnodysp$logFC < 1)))


for (i in sigRes_HTG_Colnodysp$logFC) {
  # print(i)
  if (i > 1) {
    new_row <- data.frame(regulation = "up")
    deg_reg <- rbind(deg_reg,new_row)
...
sigRes_HTG_Colnodysp_reg <- cbind(sigRes_HTG_Colnodysp, deg_reg)
nrow(sigRes_HTG_Colnodysp_reg)
```
...
```{r}
contrastMatrixColdysp <- makeContrasts(condition2colitis_dysplasia - condition2noColitis, levels = designMatrix)
...
```{r}
(res <- topTags(res, n = nrow(countTable))) # by default, sorted by adjusted Pvalue, BH correction and no adj Pvalue cutoff set
nrow(res)
(res <- topTags(res, n = nrow(countTable), adjust.method = "BH", sort.by = "PValue", p.value = 1))

# (res <- topTags(res, n = 10, adjust.method = "BH", sort.by = "logFC", p.value = 0.05))
```
...
sigRes <- subset(res$table, PValue < 0.05 & FDR < 0.05 & abs(logFC)> 1)
sigRes_HTG_Coldysp <- sigRes
knitr::kable(head(sigRes))
nrow(sigRes)

sigRes <- subset(res$table, PValue < 0.05 & abs(logFC)> 1)
nrow(sigRes)

# knitr::kable(table(FDR = nrow(sigRes1), FDRnPvalue = nrow(sigRes2), Pvalue = nrow(sigRes3)))
# ggVennDiagram(list(fdr=row.names(sigRes1), pvalue=row.names(sigRes2)))
```
...
#to confirm the number of DEG is correct
nrow(sigRes_HTG_Coldysp); sum(nrow(subset(sigRes_HTG_Coldysp,sigRes_HTG_Coldysp$logFC > 1)),
nrow(subset(sigRes_HTG_Coldysp,sigRes_HTG_Coldysp$logFC < 1)))


for (i in sigRes_HTG_Coldysp$logFC) {
  # print(i)
  if (i > 1) {
    new_row <- data.frame(regulation = "up")
    deg_reg <- rbind(deg_reg,new_row)
...
sigRes_HTG_Coldysp_reg <- cbind(sigRes_HTG_Coldysp, deg_reg)
nrow(sigRes_HTG_Coldysp_reg)
```
...
#   theme(legend.position = "none") +
#   scale_colour_manual(values = c("darkred", "grey", "steelblue")) +
#   geom_text_repel(aes(x = logFC, y = -log10(Pvalue), label = rownames(res$table)[1:10],
#                       size = 2, color = "steelblue"),
#                   data = res$table[1:10, ])
# print(volcanoPlot)
```
...
```{r}
contrastMatrix2Colnodysp <- makeContrasts(condition2colitis_nodysplasia - condition2colitisOnly, levels = designMatrix)
...
```{r}
contrastMatrix2Coldysp <- makeContrasts(condition2colitis_dysplasia - condition2colitisOnly, levels = designMatrix)
...
```{r}
contrastMatrix3Colnodysp <- makeContrasts(condition2colitis_dysplasia - condition2colitis_nodysplasia, levels = designMatrix)
...
countTable <- as.data.frame(counts(dds_cpm_filtered_HTG1))
(dge <- DGEList(countTable))
(dge <- calcNormFactors(dge))
(dge <- estimateDisp(dge, designMatrix, robust = TRUE))
fit <- glmQLFit(dge, designMatrix, robust = TRUE)
...
#to confirm the number of DEG is correct
nrow(sigRes_HTG_2_Colnodysp); sum(nrow(subset(sigRes_HTG_2_Colnodysp,sigRes_HTG_2_Colnodysp$logFC > 1)),
nrow(subset(sigRes_HTG_2_Colnodysp,sigRes_HTG_2_Colnodysp$logFC < 1)))


for (i in sigRes_HTG_2_Colnodysp$logFC) {
  # print(i)
  if (i > 1) {
    new_row <- data.frame(regulation = "up")
    deg_reg <- rbind(deg_reg,new_row)
...
sigRes_HTG_2_Colnodysp_reg <- cbind(sigRes_HTG_2_Colnodysp, deg_reg)
nrow(sigRes_HTG_2_Colnodysp_reg)
```
...
#to confirm the number of DEG is correct
nrow(sigRes_HTG_2_Coldysp); sum(nrow(subset(sigRes_HTG_2_Coldysp,sigRes_HTG_2_Coldysp$logFC > 1)),
nrow(subset(sigRes_HTG_2_Coldysp,sigRes_HTG_2_Coldysp$logFC < 1)))


for (i in sigRes_HTG_2_Coldysp$logFC) {
  # print(i)
  if (i > 1) {
    new_row <- data.frame(regulation = "up")
    deg_reg <- rbind(deg_reg,new_row)
...
sigRes_HTG_2_Coldysp_reg <- cbind(sigRes_HTG_2_Coldysp, deg_reg)
nrow(sigRes_HTG_2_Coldysp)
```
...
#to confirm the number of DEG is correct
nrow(sigRes_HTG_3_Colnodysp); sum(nrow(subset(sigRes_HTG_3_Colnodysp,sigRes_HTG_3_Colnodysp$logFC > 1)),
nrow(subset(sigRes_HTG_3_Colnodysp,sigRes_HTG_3_Colnodysp$logFC < 1)))


for (i in sigRes_HTG_3_Colnodysp$logFC) {
  # print(i)
  if (i > 1) {
    new_row <- data.frame(regulation = "up")
    deg_reg <- rbind(deg_reg,new_row)
...
sigRes_HTG_3_Colnodysp_reg <- cbind(sigRes_HTG_3_Colnodysp, deg_reg)
nrow(sigRes_HTG_3_Colnodysp)
```
...
```{r}
save.image("C:/hne_files/MSc_Bioinf/MScProject-main_C/MScProject-main_10.06.23/scripts/DEG_analysis/Gene_counts/htg_pipeline/deseq2_statistical_analysis_HTG.Rdata")
```
