...
directory <- "C:/hne_files/MSc_Bioinf/hisat2htseq_dataset/HTSeq/"



#Import data
sampleFiles <- grep("sample_[0-9]",list.files(directory),value=TRUE)
# sampleFiles <- list.files(directory)
# sample_name <- sub("(sample_[0-9]+)","\\1",sampleFiles)
sample_name <- sub("(sample_[0-9]+).*","\\1",sampleFiles)

## rearrange the samples by their sample groups, starting with coldysp,colnodysp, colonly,nocol
sampleTable <- data.frame(sampleName = sample_name,
                          fileName = sampleFiles)
all ((rownames(coldata)) %in% (sampleTable[,1]))
sampleTable[,1]
sampleTable


# Assuming sampleTable is your original table and coldata is your reference data
# Get the row names from coldata
ref_row_names <- rownames(coldata)

# Reorder the rows of sampleTable based on the row names in coldata
reordered_sampleTable <- sampleTable[ref_row_names, ], drop = FALSE]

# The resulting reordered_sampleTable will have rows in the same order as coldata
...
sampleCondition <- sub("(sample_[0-9]+).*","\\1",sampleFiles)
...
countTable <- countTable[, -grep("FFPE", colnames(countTable))]
