...
#============================== htg_dataset
matrix1 <- htg_countTable
matrix1_cpm <- cpm(matrix1)

# Which columns indices represent groups of samples?
colitis_dysplasia <- 1:10
colitis_nodysplasia <- 11:20
colitisOnly <- 21:42
noColitis <- 43:64
group_list <- list(colitis_dysplasia,colitis_nodysplasia,colitisOnly,noColitis)

keep_rows <- filter_function(matrix=matrix1_cpm, group_col_inds_list=group_list, perc=90, threshold=1)
