...
external_drive_path_htseq="/mnt/d/FASTQ_files/HTSeq/counts_merged_to_1/"

### ... ORDER OF SAMPLES BEING QUANTIFIED OUTPUT FILE
external_drive_path_sample_order="/mnt/d/FASTQ_files/HTSeq/counts_merged_to_1"


echo "running htseq-count with the UNION mode"
# to determine the order of samples: will be the column names in the output file
for i in *.bam
do 
    echo $i 
done >> $external_drive_path_sample_order/order_of_samples_quantified.txt
...
htseq-count -f bam \
            ...
            -m union \
            --nonunique none \
            *.bam $external_drive_path_gtf/genomic.gtf > $external_drive_path_sample_order/htseq_genecount_union.txt
...
echo "running the htseq-count with the INTERSECTION_NONEMPTY" mode
...
htseq-count -f bam \
            ...
            --nonunique none \
            *.bam $external_drive_path_gtf/genomic.gtf > $external_drive_path_sample_order/htseq_genecount_intersection-nonempty.txt

            
# htseq-count -f bam -s yes -t exon -i gene_id --nonunique none *.bam ../../../ref_genome_grcm39/ncbi_dataset/data/GCF_000001635.27/genomic.gtf > htseq_raw_gene_count.txt
